...
int main(int argc, char const *argv[])
{
	...
	if (is_right_argc(argc) == SUCCESS_STATUS && is_right_argv(argc, argv) == SUCCESS_STATUS)
	{
		...
		if (in_file != NULL)
		{
			...
			{
				printf("При чтение данных из файла возникли проблемы. Исправьте файл и повторите попытку.\n");
			}
		}
		else
		{
			printf("Введенный файл оказался пустым или не может быть прочтен. Исправьте и повторите попытку.\n");
		}
	}
	else
	{
		printf("Допущена ошибка в аргументах при запуске. Исправьте и повторите попытку.\n");
	}
	return errorflag;
}
