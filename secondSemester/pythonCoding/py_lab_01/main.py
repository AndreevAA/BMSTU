...
NUMBER_OF_SPACES = [5,25,14,16,15,22]  # Выделяемое количество символом для заполнения значением
TYPE_OF_ERROR = 'NULL'
EPS = ""
	...
	while temp_position < end:
		...
		if temp_position + step <= end: 
			if int(temp_position) == int(temp_position + step):
				...
				print('({:4.2f};{:4.2f}]'.format(temp_position - 0.01,temp_position + step))
			else:
		...
		if x != -1e200:
			s = __transformation__(s,'{:2.2f}'.format(x),2)
			s = __transformation__(s,'{:2.5f}'.format(function(x)),3)
